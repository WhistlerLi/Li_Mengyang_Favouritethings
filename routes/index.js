...
router.get('/', (req, res) => {
    ...
    let query = "SELECT ID, Brand, Price, Date, Country,Image FROM car";

    sql.query(query, (err, result) => {
        if (err) {throw err; console.log(err);}

        console.log("result should be:", result); // should see objects wrapped in an array
...
router.get('/users/:id', (req,res) =>{
    ...
    let query = `SELECT * FROM car WHERE ID="${req.params.id}"`;
